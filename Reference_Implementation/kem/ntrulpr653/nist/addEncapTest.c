/*
   PQCgenKAT_kem.c
   Modified from a file Modified from a file created by Bassham, Lawrence E (Fed) on 8/29/17.
...
main()
{
    ...
    unsigned char pkUnderTest[6][crypto_kem_PUBLICKEYBYTES];
    ...
    for (int i=0;i<crypto_kem_PUBLICKEYBYTES;i++){
        pkUnderTest[2][i]=0;
        if (i==crypto_kem_PUBLICKEYBYTES-1){
            pkUnderTest[2][i]=255;
        }
    }
    ...
    for (i=0; i<NUMKATS; i++) {
        ...
        fprintBstr(fp_rsp, "pk = ", pkUnderTest[i], crypto_kem_PUBLICKEYBYTES);
        
        if ( (ret_val = crypto_kem_enc(ct, ss, pkUnderTest[i])) != 0) {
